...
public class AttributeHandler {
    ...
    private void addBodyAttributes() {
        ...
        bodyUniqueAttributes.put("anchored", new UniqueAttribute("Anchored", "This weapon is really heavy and will slow the player down, but will increase your accuracy by 50%", WeaponConfig.RARITY_COMMON));

        List<String> singleShotExceptions = new ArrayList<>(); singleShotExceptions.add("Burst"); singleShotExceptions.add("Automatic");
        bodyUniqueAttributes.put("singleShot", new UniqueAttribute("Single-Shot", "This weapon fires one shot at a time", WeaponConfig.RARITY_COMMON, singleShotExceptions));

        List<String> burstExceptions = new ArrayList<>(); burstExceptions.add("Single-Shot"); burstExceptions.add("Automatic");
        bodyUniqueAttributes.put("burst", new UniqueAttribute("Burst", "This weapon fires in short bursts", WeaponConfig.RARITY_UNCOMMON, burstExceptions));

        List<String> automaticExceptions = new ArrayList<>(); automaticExceptions.add("Single-Shot"); automaticExceptions.add("Burst");
        bodyUniqueAttributes.put("automatic", new UniqueAttribute("Automatic", "This weapon fires fully automatic", WeaponConfig.RARITY_COMMON, automaticExceptions));
        bodyUniqueAttributes.put("measured", new UniqueAttribute("Measured", "The fire rate of this weapon is reduced by 40% but weapon damage is increased by 40%", WeaponConfig.RARITY_COMMON));

        // Baked, put in here for now
        bodyUniqueAttributes.put("energetic", new UniqueAttribute("Energetic", "Each shot fired by this weapon charges the weapon.  Once it is fully charged, the next shot will release a beam of compressed energy in the direction you’re aiming", WeaponConfig.RARITY_COMMON));
    }
    ...
    private void addStockAttributes() {
        ...
        stockUniqueAttributes.put("cushioned", new UniqueAttribute("Cushioned", "Every other shot has increased accuracy by x%", WeaponConfig.RARITY_COMMON));

        List<String> unstableExceptions = new ArrayList<>(); unstableExceptions.add("Overheated");
        stockUniqueAttributes.put("unstable", new UniqueAttribute("Unstable", "This weapon will have random spurts of low or high accuracy", WeaponConfig.RARITY_COMMON, unstableExceptions));

        List<String> stimulatingExceptions = new ArrayList<>(); stimulatingExceptions.add("Over-Stimulating");
        stockUniqueAttributes.put("stimulating", new UniqueAttribute("Stimulating", "x% chance that a shot fired with this weapon will restore x% of the player’s shield", WeaponConfig.RARITY_COMMON, stimulatingExceptions));

        List<String> overStimulatingExceptions = new ArrayList<>(); overStimulatingExceptions.add("Stimulating");
        stockUniqueAttributes.put("overStimulating", new UniqueAttribute("Over-Stimulating", "x% chance that a shot fired with this weapon will restore x% of the player’s shield.  If the shield is full, this can add to generating an overshield of up to half of the player’s total shield capacity", WeaponConfig.RARITY_COMMON, overStimulatingExceptions));
    }
    ...
    private void addSightAttributes() {
        ...
        sightUniqueAttributes.put("analytical", new UniqueAttribute("Analytical", "Waiting x seconds after taking a shot with this weapon will cause the next shot to have critical chance increased by x%", WeaponConfig.RARITY_COMMON));
        sightUniqueAttributes.put("tracking", new UniqueAttribute("Tracking", "x% chance for your shot to lock onto an enemy and home to them", WeaponConfig.RARITY_COMMON));
        sightUniqueAttributes.put("blurry", new UniqueAttribute("Blurry", "Accuracy and critical chance stat of this gun randomly swap while sustaining fire.  Stats go back to normal after 2 seconds of not firing", WeaponConfig.RARITY_COMMON));
    }
    ...
    private void addMagazineAttributes() {
        ...
        magazineUniqueAttributes.put("frugal", new UniqueAttribute("Frugal", "x% chance for a shot to not consume a round from the magazine", WeaponConfig.RARITY_COMMON, frugalExceptions));

        List<String> wastefulExceptions = new ArrayList<>(); wastefulExceptions.add("Frugal");
        magazineUniqueAttributes.put("wasteful", new UniqueAttribute("Wasteful", "Consumes x rounds per shot", WeaponConfig.RARITY_COMMON, wastefulExceptions));

        List<String> infernoExceptions = new ArrayList<>(); infernoExceptions.add("Heat Displacing");
        magazineUniqueAttributes.put("inferno", new UniqueAttribute("Inferno", "x% chance a shot will catch fire upon impact", WeaponConfig.RARITY_COMMON, infernoExceptions));
        magazineUniqueAttributes.put("combustion", new UniqueAttribute("Combustion", "x% chance a shot will explode upon impact", WeaponConfig.RARITY_COMMON));
        magazineUniqueAttributes.put("quickRelease", new UniqueAttribute("Quick Release", "x% chance to reload your weapon 50% faster when manually reloading", WeaponConfig.RARITY_COMMON));
        magazineUniqueAttributes.put("looping", new UniqueAttribute("Looping", "x% chance that firing the last round in your magazine will instantly restore 25% of your magazine", WeaponConfig.RARITY_COMMON));
        magazineUniqueAttributes.put("discarding", new UniqueAttribute("Discarding", "When reloading this weapon the character throws the magazine forwards.  When it lands it explodes causing area damage equal to normal weapon damage plus an additional percentage of normal weapon damage based on the percent of ammo remaining in the magazine when it was thrown", WeaponConfig.RARITY_COMMON));

        // Baked, put in here for now
        magazineUniqueAttributes.put("prismatic", new UniqueAttribute("Prismatic", "This weapon fires lasers instead of bullets.  Also receives Penetrating: Shots fired with this weapon have x% chance to penetrate what they hit", WeaponConfig.RARITY_COMMON));
    }
    ...
    private void addBarrelAttributes() {
        ...
        barrelUniqueAttributes.put("heatDisplacing", new UniqueAttribute("Heat Displacing", "As this weapon fires, the barrel heats up eventually causing shots to catch fire as they leave the barrel", WeaponConfig.RARITY_COMMON, heatDisplacingExceptions));

        // Baked, put in here for now
        barrelUniqueAttributes.put("multiShot", new UniqueAttribute("Multi-Shot", "Shots fired from this weapon release multiple projectiles at once instead of one", WeaponConfig.RARITY_COMMON));
    }

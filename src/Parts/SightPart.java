 ...
 */
public class SightPart extends Part {

    public SightPart(String name, int level, PartModel model) {
    ...
    public void generateAttributes() {
        ...
        System.out.println("Number of Sight Attributes: " + numAttributes);
        ...
        for (int x = 0; x < numAttributes; x++) {
            ...
            while (!chosen) {
                if (WeaponConfig.isNormalAttribute()) {
                    attr = AttributeHandler.instance.getNormalSightAttribute();
                }
                else {
                    boolean uniqueGet = false;

                    while (!uniqueGet) {
                        attr = AttributeHandler.instance.getUniqueSightAttribute();
